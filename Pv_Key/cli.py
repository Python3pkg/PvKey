...
from pipeline import Pipeline, Pipeline_local, Pipeline_Somatic, Pipeline_split, Pipeline_upload, Pipeline_SV
...
def SVDetect_(workflow,input_dict,**kwargs):        
    
    input_json = json.load(open(input_dict,'r'))
    inputs = [ INPUT(name='bam',path=i['path'],fmt='bam',tags=i,stage_name='Load Input Fastqs') for i in input_json ]
...
def main():
    ...
    json_lo = subparsers.add_parser('json_local',help="Local running (no parallelization). Input is a folder of FASTQs, encoded as a json file",description=json_local.__doc__,formatter_class=RawTextHelpFormatter)
    ...
    json_so = subparsers.add_parser('json_somatic',help="Parallel running of tumor-normal sample. Input is a folder of FASTQs, encoded as a json file",description=json_local.__doc__,formatter_class=RawTextHelpFormatter)
    ...
    svdetect = subparsers.add_parser('SVDetect',help="SVDetect",description=json_local.__doc__,formatter_class=RawTextHelpFormatter)
    svdetect.set_defaults(func=SVDetect_)
    cli.add_workflow_args(svdetect)
    svdetect.add_argument('-il','--input_dict',type=str,help='Inputs, see script comments for format.',required=True)
    svdetect.add_argument('-target',default='False',choices=['True','False'],help='If -target is True the MarkDuplicates step will not be performed')
